...
#This script reads paparazzi waypoints and save to GPX file and
# load a gpx file and send all coordinates to GCS
...
my $button1 = $mw -> Button(-text => "Save GPX file", -command => \&save_gpx_sub)-> pack();
my $button8 = $mw -> Button(-text => "Load GPX file", -command => \&load_gpx_sub)-> pack();

my $button2 = $mw -> Button(-text => "Get waypoints from GCS", -command => \&get_waypoints_sub)-> pack();
my $button3 = $mw -> Button(-text => "Send waypoints to GCS", -command => \&send_waypoints_sub)-> pack();
