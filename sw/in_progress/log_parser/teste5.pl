...
$mw->geometry("800x800");#$mw->geometry("200x300");
...
my $lframe = $mw->LabFrame(
	...
	)->place( -x => 8, -y => 310); #Location on the main window
...
my $table = $table_frame->Table(-columns => 9,
                                -rows => 20,
                                -fixedrows => 1,
                                -scrollbars => 'oe',
                                -relief => 'raised');

my @headers = ( "Num" , "File" , "Time" , "Lat" , "Lon" , "Alt" , "Pitch" , "Roll" , "Yaw");
