...
sub envia {
  ...
  if ($isfileloaded == 1) {
    for ($n=1; $n <= $wp_total_to_send; $n = $n + 1){
      $acid = $acid[$n];
      $lat = $lat1[$n] * 1e7;
      $lon = $lon1[$n] * 1e7;
      $alt = $alt1[$n] * 100;
      $reply = "$distantAppli MOVE_WP $n $acid $lat $lon $alt";
      printf "$reply \n";
...
##############################  PRINT WAYPOINT FILE ON CONSOLE ###############################

sub lista {
my $n;
for ($n=1; $n <= $wp_max; $n = $n + 1){
  printf "Wpt:%d - ac_id:%d lat:%f lon:%f alt:%d\n",$n, $acid[$n], $lat[$n],$lon[$n],$alt[$n];
  }
}
...
my $types = [ ['Waypoint files', '.wpt'],
              ['Txt files', '.txt'],
              ['All Files',   '*'],];  

my $sfile = $mw->getSaveFile( -title => 'Save Waypoint File:',-filetypes => $types,
                              -defaultextension => '.wpt', -initialdir => '~/Desktop/waypoint_files');
open WPFILE, ">$sfile" or die $!;
printf WPFILE "wp_max $wp_max\n";
for ($n=1; $n <= $wp_max; $n = $n + 1){
  printf "Wpt:%d - ac_id:%d lat:%f lon:%f alt:%d\n",$n, $acid[$n], $lat[$n],$lon[$n],$alt[$n];
  printf WPFILE "wpt %d %d %f %f %d\n",$n,$acid[$n],$lat[$n],$lon[$n],$alt[$n];
  }
  print WPFILE "wp_total $wp_max\n";
close WPFILE;  
}
...
my $types = [ ['Waypoint files', '.wpt'],
              ['Txt files', '.txt'],
              ['All Files',   '*'],];  
#my $fname = $mw->getOpenFile( -title => 'Open Waypoint File:',-filetypes => $types, -defaultextension => '.wpt', -initialdir => '~/Desktop/waypoint_files' );
my $fname = $mw-> LoadDialog->Show();
...
while (my $line = <WPFILE>) {
  ...
  if ($fields[0] eq "wp_max") {$wp_max = $fields[1];}
  if ($fields[0] eq "wp_total") {$wp_total = $fields[1];}
  if ($fields[0] eq "wpt") {
    $wp_num = $fields[1];
    $acid = $fields[2];
    $lat = $fields[3];
